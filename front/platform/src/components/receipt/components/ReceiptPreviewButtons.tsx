...
interface ReceiptPreviewButtonsProps {
  judgSeq: number
}

const ReceiptPreviewButtons: React.FC<ReceiptPreviewButtonsProps> = ({ judgSeq }) => (
  <div className="mt-6 flex justify-center gap-5 p-3">
    ...
    >
      <Link to={`/receipt/application/${judgSeq}`} state={{ isWrite: true }}>
        {RECEIPT_BUTTON_TEXTS.RECEIPT_REGISTRATION}
      </Link>
    </button>
  </div>
)

export default ReceiptPreviewButtons
