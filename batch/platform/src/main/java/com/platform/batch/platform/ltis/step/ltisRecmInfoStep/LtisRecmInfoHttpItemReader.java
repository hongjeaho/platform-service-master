...
import com.platform.batch.platform.ltis.dto.LtisRecmInfoResponse;
...
@Component
@StepScope
public class LtisRecmInfoHttpItemReader implements ItemStreamReader<List<LtisRecmInfoDto>> {
  ...
  public static final String CURRENT_INDEX = "ltis.recmInfo.currentIndex";


  public LtisRecmInfoHttpItemReader(
  ...
  public void open(@NonNull ExecutionContext executionContext) throws ItemStreamException {
    try {
      List<Long> judgSeqList = ltisDataMapper.getRecmJudgSeq();
      ...
      if (currentIndex == 0 && !judgSeqList.isEmpty()) {
        ltisDataMapper.deleteRecmInfo(judgSeqList);
      }

      this.iterator = judgSeqList.listIterator(currentIndex);
      log.info("[LTIS-RECMINFO-READER] open: totalTargets={}, checkpointIndex={}", judgSeqList.size(), currentIndex);
    } catch (Exception e) {
      throw new ItemStreamException("Failed to open LtisRecmInfoHttpItemReader", e);
    }
  }

  @Override
  public List<LtisRecmInfoDto> read() {
    while (true) {
      if (!iterator.hasNext()) {
        return null; // 끝
      }
      ...
      String body = ltisWebClient.get()
          .uri(b -> b.path(LtisApiPath.RECM_INFO.getPath())
      ...
      var resp = LtisRecmInfoResponse.ofResponse(body);
      if (!"200".equals(resp.getCode())) {
        // 정책상 4xx/5xx 중 실패 처리
        log.error("LTIS RECM INFO HTTP error, code={}, judgSeq={}", resp.getCode(), judgSeq);
        continue;
      }
      var dto = resp.getRecmInfoList();
      if (dto == null) {
        // 데이터 없음: 다음 아이템으로 진행 (스킵)
        log.warn("[LTIS-RECM-INFO-READER] empty detail for judgSeq={} -> skip", judgSeq);
        continue;
      }
      return dto;
    }
